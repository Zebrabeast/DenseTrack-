import torch
from MPM_Net import MPMNet
import numpy as np
import cv2
import os
import matplotlib.pyplot as plt
from scipy.optimize import linear_sum_assignment
import h5py


class trackp():
    def __init__(self, mag_th, itp, sigma, maxv, image_size):
        self.MAG_TH = mag_th
        self.ATP_UTV = itp
        self.SIGMA = sigma
        self.MAXV = maxv
        self.IMAGE_SIZE = image_size

    def loadModel(self, model_path, parallel=False):
        '''load UNet-like model
        Args:
             model_path (str): model path (.pth)
             parallel (bool): for multi-gpu
        Return:
            model
        '''

        # model = MPMNet(2, 3)
        model = MPMNet(6, 3)  #  这里的MPMNet从2，3  改为6，3有什么意义
        model.cuda()
        # state_dict = torch.load(model_path)
        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        checkpoint = torch.load(model_path)
        if parallel:
            model = torch.nn.DataParallel(model)
        # model.load_state_dict(state_dict)
        model.load_state_dict(checkpoint['state_dict'])
        model.eval()
        return model

    # def load_fame0(self, coor_list_path):
    #     f = h5py.File(coor_list_path, 'r')
    #     coor_list = f['coordinate']
    #     fidt = f['d6']
    #     return coor_list, fidt

    def inferenceMPM(self, names, model):
        '''
        Args:
             names (list of str): list of image path
             model (nn.Module): torch model
        Return:
            mpm (numpy.ndarray): mpm (HEIGHT, WIDTH, 3), ch0: y_flow, ch1: x_flow, ch2: z_flow
        '''

        # imgs = [(cv2.imread(name, -1) / self.MAXV).astype('float32')[None] for name in names]
        # print(cv2.imread(names[0], -1).shape)
        # print(imgs.shape)
        # imgs = [cv2.normalize(cv2.imread(name), None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32F) for name in names]
        # imgs = []
        # for name in names:
        #     imgs.append(cv2.normalize(cv2.imread(name), None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32F))
        # img = np.concatenate(imgs, axis=0)
        # img = np.concatenate(imgs, axis=-1)

        img1 = cv2.imread(names[0], -1)
        img2 = cv2.imread(names[1], -1)
        if len(img1.shape) == 2:
            img1 = np.expand_dims(img1, axis=2)
            img2 = np.expand_dims(img2, axis=2)
        img = np.concatenate([img1, img2], axis=-1)
        if img.max() > 1:
            img = img / 255
        img = img.transpose((2, 0, 1))
        print(img.shape)
        img = torch.from_numpy(img).unsqueeze(0).type(torch.FloatTensor)
        print(img.shape)
        img = img.cuda()
        # img = img.type(torch.FloatTensor)
        output = model(img)
        print()
        mpm = output[0].cpu().detach().numpy()
        mpm = np.transpose(mpm, axes=[1, 2, 0])
        return mpm

    def getMPMpoints(self, mpm, mag, mag_max=1.0):
        '''
        Args:
            mpm (numpy.ndarray): MPM
            mag (numpy.ndarray): Magnitude of MPM i.e. heatmap
            mag_max: Maximum value of heatmap
        Return:
            result (numpy.ndarray): Table of peak coordinates, warped coordinates, and peak value [x, y, wx, wy, pv]
        '''
        mag[mag > mag_max] = mag_max
        map_left_top, map_top, map_right_top = np.zeros(mag.shape), np.zeros(mag.shape), np.zeros(mag.shape)
        map_left, map_right = np.zeros(mag.shape), np.zeros(mag.shape)
        map_left_bottom, map_bottom, map_right_bottom = np.zeros(mag.shape), np.zeros(mag.shape), np.zeros(mag.shape)
        map_left_top[1:, 1:], map_top[:, 1:], map_right_top[:-1, 1:] = mag[:-1, :-1], mag[:, :-1], mag[1:, :-1]
        map_left[1:, :], map_right[:-1, :] = mag[:-1, :], mag[1:, :]
        map_left_bottom[1:, :-1], map_bottom[:, :-1], map_left_bottom[1:, :-1] = mag[:-1, 1:], mag[:, 1:], mag[1:, 1:]
        peaks_binary = np.logical_and.reduce((
            mag >= map_left_top, mag >= map_top, mag >= map_right_top,
            mag >= map_left, mag > self.MAG_TH, mag >= map_right,
            mag >= map_left_bottom, mag >= map_bottom, mag >= map_right_bottom,
        ))
        _, _, _, center = cv2.connectedComponentsWithStats((peaks_binary * 1).astype('uint8'))
        center = center[1:]

        result = []
        for center_cell in center.astype('int'):
            # vec = mpm[center_cell[1], center_cell[0]]
            # mag_value = mag[center_cell[1], center_cell[0]]
            # # vec = vec / np.linalg.norm(vec)
            # x = 10000 if vec[2] == 0 else int(5 * (vec[1] / vec[2]))  # Calculate the motion vector x
            # y = 10000 if vec[2] == 0 else int(5 * (vec[0] / vec[2]))  # Calculate the motion vector y
            # , center_cell[0] + x, center_cell[1] + y, mag_value
            result.append([center_cell[0], center_cell[1]])
        return result


    def associateObjs(self, frame, pos, pre_pos, mag, new_id):
        ass_tab = []
        ass_flag = np.zeros(len(pos))
        fin_flag = np.ones(len(pre_pos))
        pos = np.array(pos)
        for i, focus_pos in enumerate(pre_pos):
            x, y, fmag_value, mag_value = self._movePeak(mag, focus_pos[4], focus_pos[5])
            if mag_value > self.MAG_TH:
                # distance_list = [np.sqrt((pospos[2] - x) ** 2 + (pospos[3] - y) ** 2) for pospos in pre_pos]
                # import pdb
                # pdb.set_trace()
                distance_list = np.linalg.norm(pos[:, 0:2] - np.array([[x, y]]), axis=1).tolist()
                if min(distance_list) < 10:
                    add_ass_df = []
                    min_idx = np.argmin(distance_list)
                    add_ass_df.extend([frame, focus_pos[1], pos[min_idx][0], pos[min_idx][1], pos[min_idx][2], pos[min_idx][3], pos[min_idx][4]])
                    add_ass_df.extend([fmag_value, mag_value, min(distance_list), focus_pos[10]])
                    ass_tab.append(add_ass_df)
                    fin_flag[i] = 0
                    ass_flag[min_idx] = 1
        print(f'# of associated cell: {len(ass_tab)}')

        # appearance part --------------------------------------------------------
        new_pos = np.delete(pos, np.where(ass_flag != 0)[0], axis=0).tolist()
        app_tab = []
        for focus_new_pos in new_pos:
            add_new_df = [frame, new_id] + focus_new_pos + [0, 0, 0, 0]
            ass_tab.append(add_new_df)
            app_tab.append(add_new_df)
            new_id += 1
        # import pdb
        # pdb.set_trace()
        ass_tab = np.array(ass_tab)
        print(f'appeared cell: {len(app_tab)}')

        # division part ----------------------------------------------------------
        # 这一部分需要加一段代码，如果associated cell是0的话，不需要这样判断。
        div_tab = np.empty((0, 11))
        if len(ass_tab) > 0:
            ids, count = np.unique(ass_tab[:, 1], axis=0, return_counts=True)
            two_or_more_ids = ids[count > 1]
            for tm_id in two_or_more_ids:
                d_candidates = ass_tab[ass_tab[:, 1] == tm_id]
                d_candidates = d_candidates[np.argsort(d_candidates[:, 7])]
                # Change not daughters id & add not daughters to app tab -------------
                for d in d_candidates[:-2]:
                    d[1] = new_id
                    ass_tab[:, 1][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = new_id
                    app_tab.append(d.tolist())
                    new_id += 1
                # Change daughters id & register parents id -------------------------
                for d in d_candidates[-2:]:
                    ass_tab[:, 1][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = new_id
                    ass_tab[:, 10][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = tm_id
                    new_id += 1
                div_tab = np.append(div_tab, ass_tab[ass_tab[:, 10] == tm_id], axis=0)
            print(f'division cell: {len(div_tab)}')


        not_ass_tab = np.delete(pre_pos, np.where(fin_flag == 0)[0], axis=0)
        print(f'not associated cell: {len(not_ass_tab)}')

        return ass_tab, not_ass_tab, div_tab, np.array(app_tab), new_id


    def motion_associate(self,dets_one, trks,track_indices,mag,distance_threshold):
        
        pred_pos = [traj.predicted_coordinate for traj in trks.values()]
        pos = dets_one["coordinate"]

        # # 定义距离阈值
        # DISTANCE_THRESHOLD = 10.0
        distance_matrix = self.get_motion_scores(pred_pos, pos, mag, distance_threshold = 10)
        modified_matrix = np.sqrt(distance_matrix)
        # 现在distance_matrix中包含了所有预测位置与检测位置之间的分数+-
        # 分数越高表示匹配度越好
        # 应用匈牙利算法找到最优匹配
        row_indices, col_indices = linear_sum_assignment(-distance_matrix)  # 取负是因为匈牙利算法默认求最小值
        # 一种处理方式哈，感觉是不是变化太剧烈了比如要想到达0.9，距离不能超过 1.5，虽然是预测与检测之间的比对
        # 那么我 开根号再*10，就是那个提高平均分的那个 这样的话，0.5分数变为0.7，有个弊端 就是距离产生的影响不那么剧烈
        # 获取匹配结果   前面的过滤是将超过阈值的设为0，便于最大化匹配，但是部分并没有除去，还需要再判断
        # 感觉可行，对于阈值 10 来说，4，6 差距基本不大，但是0.4  0.6差距就有点大了，但是有个担心事，就是他的差距可能不大，
        # 这样就根本没意义
        # modified_matrix = np.sqrt(distance_matrix) * 10
        
        matched_pairs = []
        for det_idx, trk_idx in zip(row_indices, col_indices):
            score = modified_matrix[det_idx, trk_idx]
            if score > 0.5:  # 只考虑有效匹配（分数大于）分数大于暂定于0.7,相当于距离为5 害, 0.5相当于7-8之间
                matched_pairs.append((det_idx, track_indices[trk_idx], score))
        print(f'# motions_match of associated people: {len(matched_pairs)}')
        
        matched_pairs = np.array(matched_pairs)  # 形状为 (N, 3)
        unmatched_detections = []
        for d in range(len(pos)):
            if d not in matched_pairs[:, 0]:
                unmatched_detections.append(d)
    
        unmatched_trackers = []
        for t in trks.keys(): #这里统一使用key，方便后面对于traj进行处理。
            if t not in matched_pairs[:, 1]:
                unmatched_trackers.append(t)

        return (np.array(matched_pairs),np.array(unmatched_detections),np.array(unmatched_trackers)),modified_matrix

    # def get_motion_scores(self,pred_pos, pos, mag, distance_threshold):

    #     # 定义距离阈值
    #     DISTANCE_THRESHOLD = distance_threshold  #严格的可以设置为 10
    #     # 创建距离矩阵 - 尺寸为 len(pos) × len(pred_pos)
    #     distance_matrix = np.zeros((len(pos), len(pred_pos)))
    #     print(f"motions_match scores------len(pos): {len(pos)},len(pred_pos):{len(pred_pos)}")
    #     num = 0
    #     # 假设 pred_pos 是原始预测点 (N, 2)
    #     # 初始化坐标差列表
    #     offset_list = []
    #     valid_indices = []
    #     # 计算每个预测位置和检测位置之间的距离矩阵
    #     for i, focus_pos in enumerate(pred_pos):
    #         x, y, fmag_value, mag_value = self._movePeak(mag, focus_pos[0], focus_pos[1])
    #         # x,y = focus_pos[0],focus_pos[1]
    #         if mag_value > self.MAG_TH: #  这里略微调整一下  经过输出检测，这里会过滤掉一下点 self.MAG_TH
    #             dx = x - focus_pos[0]
    #             dy = y - focus_pos[1]
    #             offset = np.sqrt(dx**2 + dy**2)
    #             offset_list.append(offset)
    #             valid_indices.append(i)  # 保留原始点的编号
    #             # 计算当前预测位置与所有检测位置的距离
    #             distances = np.linalg.norm(pos[:, 0:2] - np.array([[x, y]]), axis=1)
    #             num += 1
    #             # 将距离转换为分数: (阈值-距离)/阈值
    #             # 距离越小，分数越高；超过阈值的设为0   后面可以动调整，比如开根号*10
    #             scores = np.where(distances < DISTANCE_THRESHOLD,
    #                               (DISTANCE_THRESHOLD - distances) / DISTANCE_THRESHOLD,
    #                               0)
    #             # 将分数存入距离矩阵
    #             distance_matrix[:, i] = scores
    #     print("the num of obey mag_pos:",num)

    #      # 转为 numpy 数组
    #     offset_array = np.array(offset_list)
    #     index_array = np.array(valid_indices)

    #             # 画散点图：每个点独立显示，无连接线
    #     plt.figure(figsize=(12, 5), dpi=200)
    #     plt.scatter(index_array, offset_array, color='blue', s=10)  # s 控制点的大小

    #     plt.title("Offset Magnitude per Point (No Connection)")
    #     plt.xlabel("Index in pred_pos")
    #     plt.ylabel("Offset Magnitude (pixels)")
    #     plt.grid(True)

    #     # 控制 x 轴范围与刻度
    #     plt.xlim(index_array.min(), index_array.max())
    #     plt.xticks(np.linspace(index_array.min(), index_array.max(), num=10, dtype=int))
    #     # 保存图像
    #     save_dir = "./vis_results"
    #     os.makedirs(save_dir, exist_ok=True)
    #     os.makedirs(save_dir, exist_ok=True)
    #     if len(pos)==230:
    #         save_path = os.path.join(save_dir, "peak_offset_per_point.png")
    #         plt.savefig(save_path, dpi=300, bbox_inches='tight') 
    #     return distance_matrix
    def get_motion_scores(self,pred_pos, pos, mag, distance_threshold):

        # 定义距离阈值
        DISTANCE_THRESHOLD = distance_threshold  #严格的可以设置为 10
        # 创建距离矩阵 - 尺寸为 len(pos) × len(pred_pos)
        distance_matrix = np.zeros((len(pos), len(pred_pos)))
        print(f"motions_match scores------len(pos): {len(pos)},len(pred_pos):{len(pred_pos)}")
        num = 0
        # 计算每个预测位置和检测位置之间的距离矩阵
        for i, focus_pos in enumerate(pred_pos):
            # x, y, fmag_value, mag_value = self._movePeak(mag, focus_pos[0], focus_pos[1])
            x,y = focus_pos[0],focus_pos[1]
            # if mag_value > self.MAG_TH: #  这里略微调整一下  经过输出检测，这里会过滤掉一下点 self.MAG_TH
            # 计算当前预测位置与所有检测位置的距离
            distances = np.linalg.norm(pos[:, 0:2] - np.array([[x, y]]), axis=1)
            num += 1
            # 将距离转换为分数: (阈值-距离)/阈值
            # 距离越小，分数越高；超过阈值的设为0   后面可以动调整，比如开根号*10
            scores = np.where(distances < DISTANCE_THRESHOLD,
                                (DISTANCE_THRESHOLD - distances) / DISTANCE_THRESHOLD,
                                0)
            # 将分数存入距离矩阵
            distance_matrix[:, i] = scores
        print("the num of obey mag_pos:",num)
        return distance_matrix

        # 定义距离阈值
        DISTANCE_THRESHOLD = distance_threshold  #严格的可以设置为 10
        # 创建距离矩阵 - 尺寸为 len(pos) × len(pred_pos)
        distance_matrix = np.zeros((len(pos), len(pred_pos)))
        print(f"motions_match scores------len(pos): {len(pos)},len(pred_pos):{len(pred_pos)}")
        num = 0
        # 计算每个预测位置和检测位置之间的距离矩阵
        for i, focus_pos in enumerate(pred_pos):
            # x, y, fmag_value, mag_value = self._movePeak(mag, focus_pos[0], focus_pos[1])
            x,y = focus_pos[0],focus_pos[1]
            if mag_value > self.MAG_TH: #  这里略微调整一下  经过输出检测，这里会过滤掉一下点 self.MAG_TH
                # 计算当前预测位置与所有检测位置的距离
                distances = np.linalg.norm(pos[:, 0:2] - np.array([[x, y]]), axis=1)
                num += 1
                # 将距离转换为分数: (阈值-距离)/阈值
                # 距离越小，分数越高；超过阈值的设为0   后面可以动调整，比如开根号*10
                scores = np.where(distances < DISTANCE_THRESHOLD,
                                  (DISTANCE_THRESHOLD - distances) / DISTANCE_THRESHOLD,
                                  0)
                # 将分数存入距离矩阵
                distance_matrix[:, i] = scores
        print("the num of obey mag_pos:",num)
        return distance_matrix


    def associateCells(self, frame, pos, pre_pos, pre_mag, new_id):
        '''Associate objects
         Args:
             frame (int): current frame
             pos (array-like): position table (from self.getMPMpoints)
             pre_pos (array-like) previous position table
             pre_mag (numpy.ndarray): previous heatmap
             new_id (int): new id for new cells
        Returns:
            ass_tab: record of associated cells
            not_ass_tab: record of not associated cells
            div_tab: record of mitosis cells
            app_tab: record of appeared cells
            new_id (int): new id for new cells
        '''

        ass_tab = []
        ass_flag = np.zeros(len(pos))
        fin_flag = np.ones(len(pre_pos))

        # association part -------------------------------------------------------
        # process each position of current frame ---------------------------------
        for i, focus_pos in enumerate(pos):
            # x, y is estimated point moved to the peak
            x, y, fmag_value, mag_value = self._movePeak(pre_mag, focus_pos[2], focus_pos[3])
            if mag_value > self.MAG_TH:
                # distance_list = [np.sqrt((pospos[2] - x) ** 2 + (pospos[3] - y) ** 2) for pospos in pre_pos]
                distance_list = np.linalg.norm(pre_pos[:, 2:4] - np.array([[x, y]]), axis=1).tolist()
                if min(distance_list) < 10:
                    add_ass_df = []
                    min_idx = np.argmin(distance_list)
                    add_ass_df.extend([frame, pre_pos[min_idx][1]] + focus_pos)
                    add_ass_df.extend([fmag_value, mag_value, min(distance_list), pre_pos[min_idx][10]])
                    ass_tab.append(add_ass_df)
                    fin_flag[min_idx] = 0
                    ass_flag[i] = 1
        print(f'# of associated cell: {len(ass_tab)}')

        # appearance part --------------------------------------------------------
        new_pos = np.delete(pos, np.where(ass_flag != 0)[0], axis=0).tolist()
        app_tab = []
        for focus_new_pos in new_pos:
            add_new_df = [frame, new_id] + focus_new_pos + [0, 0, 0, 0]
            ass_tab.append(add_new_df)
            app_tab.append(add_new_df)
            new_id += 1
        ass_tab = np.array(ass_tab)
        print(f'appeared cell: {len(app_tab)}')

        # division part ----------------------------------------------------------
        # 这一部分需要加一段代码，如果associated cell是0的话，不需要这样判断。
        div_tab = np.empty((0, 11))
        if len(ass_tab) > 0:
            ids, count = np.unique(ass_tab[:, 1], axis=0, return_counts=True)
            two_or_more_ids = ids[count > 1]
            for tm_id in two_or_more_ids:
                d_candidates = ass_tab[ass_tab[:, 1] == tm_id]
                d_candidates = d_candidates[np.argsort(d_candidates[:, 7])]
                # Change not daughters id & add not daughters to app tab -------------
                for d in d_candidates[:-2]:
                    d[1] = new_id
                    ass_tab[:, 1][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = new_id
                    app_tab.append(d.tolist())
                    new_id += 1
                # Change daughters id & register parents id -------------------------
                for d in d_candidates[-2:]:
                    ass_tab[:, 1][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = new_id
                    ass_tab[:, 10][(ass_tab[:, 2] == d[2]) & (ass_tab[:, 3] == d[3])] = tm_id
                    new_id += 1
                div_tab = np.append(div_tab, ass_tab[ass_tab[:, 10] == tm_id], axis=0)
            print(f'division cell: {len(div_tab)}')
        return ass_tab

    def _movePeak(self, mag, x, y):
        max_idx = -1
        try:
            first_mag_value = mag[int(y), int(x)]
            # 额，这里进行略微修改，这里虽然mag 不大，但并不能代表他没价值，还要后面再看
            if first_mag_value < self.MAG_TH:
                return int(x), int(y), 0, (self.MAG_TH + 0.01)
        except IndexError:
            return int(x), int(y), 0, 0
        while max_idx != 2:
            points = [[y + 1, x], [y - 1, x], [y, x], [y, x - 1], [y, x + 1]]
            mags = []
            for p in points:
                try:
                    mags.append(mag[int(p[0]), int(p[1])])
                except IndexError:
                    mags.append(0)  # out of frame
            max_idx = np.argmax(mags)
            y, x = points[max_idx]
            if mags[max_idx] == mags[2]:
                break
        x, y = self._adjastPosition(x, y)
        return x, y, first_mag_value, mags[max_idx]

    def _adjastPosition(self, x, y):
        y = 0 if y < 0 else y
        y = self.IMAGE_SIZE[0] - 1 if y > self.IMAGE_SIZE[0] - 1 else y
        x = 0 if x < 0 else x
        x = self.IMAGE_SIZE[1] - 1 if x > self.IMAGE_SIZE[1] - 1 else x
        return int(x), int(y)

    def _reformatID(self, ids, log):
        new_ids = np.arange(1, len(ids) + 1)
        new_log = np.empty((0, 11))
        for each_log in log.copy():
            eid = each_log[1]
            pid = each_log[10]
            each_log[1] = new_ids[np.where(ids == eid)[0][0]]
            if pid != 0:
                try:
                    each_log[10] = new_ids[np.where(ids == pid)[0][0]]
                except:
                    print(pid)
            new_log = np.append(new_log, [each_log], axis=0)
        return new_log

    def saveCTK(self, log, not_ass_log, div_log, save_dir):
        log = log[np.argsort(log[:, 0])]
        ids = np.unique(log[:, 1])

        new_log = self._reformatID(ids, log)
        # new_not_ass_log = self.reformatID(ids, not_ass_tab)
        # new_div_log = self.reformatID(ids, div_tab)
        # new_ins_log = self.reformatID(ids, ins_log)

        save_dir = os.path.join(save_dir, 'track_log')
        os.makedirs(save_dir, exist_ok=True)
        np.savetxt(os.path.join(save_dir, 'new_log.num_frame'), new_log)
        np.savetxt(os.path.join(save_dir, 'tab.num_frame'), log)
        np.savetxt(os.path.join(save_dir, 'end_trajectory.num_frame'), not_ass_log)
        np.savetxt(os.path.join(save_dir, 'mitosis_event.num_frame'), div_log)

        np.savetxt(os.path.join(save_dir, 'tracking.states'), new_log[:, :4], fmt='%d')
        np.savetxt(os.path.join(save_dir, 'end_trajectory.num_frame'), not_ass_log, fmt='%d')
        np.savetxt(os.path.join(save_dir, 'mitosis_event.num_frame'), div_log, fmt='%d')

        tree = np.empty((0, 2))
        parents = np.unique(new_log[:, 10])
        for nid in np.unique(new_log[:, 1]):
            par = new_log[:, 10][new_log[:, 1] == nid][0]
            tree_id = [nid, par]
            tree = np.append(tree, [tree_id], axis=0)
        np.savetxt(os.path.join(save_dir, 'tracking.tree'), tree, fmt='%d')