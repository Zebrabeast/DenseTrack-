...
def get_args_parser():
    ...
    # misc parameters
    parser.add_argument('--device', default='cuda',    #这里使用第二张GPU
...
def evaluate_single_image(model, img_path, device, vis_dir=None):
    ...
    if vis_dir: 
        ...
        # 保存 points 到 .txt 文件
        #鉴于对于部分输出为零，这里会报错故特殊处理，先注释掉
        # dist_thresh = 12.0  #优化输出，除去置信度低的预测点
        ...
        visualization1(samples, [points], vis_dir, img_path, split_map=split_map)

def main(args):
    
    # input image and model
    args.img_dir = '/home/data_SSD/zk/dataset/00074/origin'
    args.resume = 'best_checkpoint.pth'
    args.vis_dir = '/home/data_SSD/zk/pet_outputs/special_case/00074'
    ...
    image_files = [f for f in os.listdir(args.img_dir) if f.endswith(('.jpg', '.png', '.jpeg'))]
    
    # Process each image
    for image_file in image_files:
        img_path = os.path.join(args.img_dir, image_file)
        print(f"Processing {img_path}...")
        
        # evaluation
        vis_dir = None if args.vis_dir == "" else args.vis_dir
        evaluate_single_image(model, img_path, device, vis_dir=vis_dir)
