...
class DRONE(Dataset):
    def __init__(self, data_root, transform=None, train=False, flip=False):
        ...
        self.prefix = prefix
        # 根据DroneCrowd数据集进行改变
        self.img_list = os.listdir(f"{data_root}/{prefix}/{prefix}/images")

        # get image and ground-truth list
        self.gt_list = {}
        for img_name in self.img_list:
            img_path = f"{data_root}/{prefix}/{prefix}/images/{img_name}"
            gt_path = f"{data_root}/{prefix}/{prefix}/ground_truth/GT_{img_name}"
...
def load_data(img_gt_path, train):
    ...
    # 这一块可以根据FIDT对照着改
    points = io.loadmat(gt_path)['image_info'][0, 0][0, 0][0][:, :-1][:, ::-1]
...
def build(image_set, args):
    ...
    if image_set == 'train':
        train_set = DRONE(data_root, train=True, transform=transform, flip=True)
        return train_set
    elif image_set == 'val':
        val_set = DRONE(data_root, train=False, transform=transform)
        return val_set
